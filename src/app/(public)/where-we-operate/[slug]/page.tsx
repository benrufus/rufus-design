...
// Service words that cycle after the town name
...
export default async function LocationPage({ params }: Props) {
  ...
  // Town first, then cycle through services
    ...
    <>
      ...
      <main>
        ...
        {loc.services?.length > 0 && (
          <section style={{ padding: '2rem clamp(1.5rem,4vw,3rem)', background: 'var(--bg)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {loc.services.map((s: string) => (
                <span key={s} style={{ fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', background: 'rgba(255,128,0,0.15)', color: '#ff8000', border: '1px solid rgba(255,128,0,0.3)', padding: '0.35em 0.85em', fontFamily: 'var(--font-raleway)', fontWeight: 700 }}>{s}</span>
              ))}
            </div>
          </section>
        )}
        ...
        {loc.body?.length > 0 && (
          <article style={{ padding: '3rem clamp(1.5rem,4vw,3rem) 6rem', display: 'grid', gridTemplateColumns: '220px 1fr', gap: '4rem', alignItems: 'start', maxWidth: '1100px' }}>
            <aside>
              <TableOfContents />
              <a href="/contact" className="btn-primary" style={{ display: 'block', textAlign: 'center', marginTop: '1.5rem' }}>Get in touch</a>
            </aside>
        ...
        {/* Back link */}
        <div style={{ padding: '0 clamp(1.5rem,4vw,3rem) 3rem' }}>
          <a href="/where-we-operate" style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'none', fontSize: '0.85rem' }}>
            ← All areas
          </a>
        </div>
